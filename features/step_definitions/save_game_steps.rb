...
Given("{string} is in the index page") do |game_title|
  ...
  Game.create(game_title: game_title)
end
...
And("I have not already saved the game {string}") do |game_title|
  ...
  user = User.first
  saved_game = SavedGame.find_by(user: user, game: game)
...
And("I have already saved the game {string}") do |game_title|
  game = Game.find_by(game_title: game_title)
  user = User.first
  SavedGame.create(user: user, game: game)
end

When("I click on the Save button for the game {string}") do |game_title|
  click_button("Save")
end
