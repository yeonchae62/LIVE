...
Given('I am on the games with publication-year page') do
  ...
  Then('I should see a list of latest games') do
    ...
    games = Game.where('publication_year LIKE ?', '____').order(publication_year: :desc)
    games.each do |game|
  ...
  Then('I should see a list of game needed to be checked') do
    ...
    games = Game.where.not(publication_year: nil).select { |game| game.publication_year.to_s.match?(/\d/) && game.publication_year.to_s.length != 4 }
    games.each do |game|
      expect(page).to have_content(game.publication_year.presence || 'info not included')
    end
  end
